...
write_read_tx_test() ->
    ...
    LoadedBundle = hb_cache:ensure_all_loaded(Bundle, Opts),
    ?assert(hb_message:verify(LoadedBundle, all, Opts)),
    {ok, Child} =
        hb_ao:resolve(
            LoadedBundle,
            <<"1/2">>,
...
stale_ans104_offset_returns_error_test() ->
    Store = [hb_test_utils:test_store()],
    Opts = #{<<"store">> => Store, <<"index-store">> => Store},
    ...
    ?assertMatch({error, {id_mismatch, _}}, Result).
